...
public:
    ...
    Personal(int id, int varsta, const char* nume, char tura, const char sectie[100], float inaltime, double greutate, bool garzi, int* Ore, int zileLucrate) :ID(id)
    {
        cout << ID << endl;
        this->varsta = varsta;
    ...
    {
        out << "\nPersonalul " << pers.nume << " cu varsta de " << pers.varsta << " are " << pers.zileLucrate << "  zile lucrate, in aceasta saptamana " << endl;
        out << " la sectia de " << pers.sectie << " lucrand tura de " << pers.tura << endl;
