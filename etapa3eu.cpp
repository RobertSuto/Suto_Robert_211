...
public:
    ...
    friend bool operator == (Medicament, Medicament);
    friend bool operator < (Medicament, Medicament);
    friend bool operator > (Medicament, Medicament);
};

bool operator == (Medicament m1, Medicament m2)
{
    if (m1.producator != m2.producator || m1.cantitate != m2.cantitate || m1.pret != m2.pret || m1.antibiotic != m2.antibiotic)
        return false;
    return true;
}
...
protected:
    ...
    list<Dezinfectant*> dezinfectanti;
    set<Medicament> medicamente;
public:
    //SETERS & GETTERS
    int getID()
    {
    ...
    Personal(const Personal& l2) :ID(l2.ID)
    {
        ...
        for (Medicament med : l2.medicamente)
        {
            this->medicamente.insert(med);
        }
        
        for (Dezinfectant* dez : l2.dezinfectanti)
        {
            this->dezinfectanti.push_back(dez);

            DezinfectantBacterii* p = dynamic_cast<DezinfectantBacterii*>(dez);
            if (p != nullptr)
            {
                this->dezinfectanti.push_back(new DezinfectantBacterii(*p));
            }
            DezinfectantVirusuri* p2 = dynamic_cast<DezinfectantVirusuri*>(dez);
            if (p2 != nullptr)
            {
                this->dezinfectanti.push_back(new DezinfectantVirusuri(*p2));
            }
        }
    ...
    Personal& operator= (const Personal& pers)
    {
        ...
        for (Medicament med : pers.medicamente)
        {
            /*this->medicamente.insert(med);*/
        }
        
        for (Dezinfectant* dez : pers.dezinfectanti)
        {
            DezinfectantBacterii* p = dynamic_cast<DezinfectantBacterii*>(dez);
            if (p != nullptr)
            {
    ...
    {
        /*this->medicamente.insert(m);*/
    }
    void adaugaDezinfectant(Dezinfectant* d)
    {
        DezinfectantBacterii* p = dynamic_cast<DezinfectantBacterii*>(d);
        if (p != nullptr)
        {
            this->dezinfectanti.push_back(new DezinfectantBacterii(*p));
        }
        DezinfectantVirusuri* p2 = dynamic_cast<DezinfectantVirusuri*>(d);
        if (p2 != nullptr)
        {
            this->dezinfectanti.push_back(new DezinfectantVirusuri(*p2));
        }
    ...
    friend ostream& operator<< (ostream& out, const Personal& pers)
    {
        ...
        for (const Dezinfectant* dez : pers.dezinfectanti)
        {
            out << *dez << ' ';
        }
       ...
       /* for (int i = 0; i < pers.medicamente.size(); i++)
        ...
        for (const Medicament& med : pers.medicamente)
        {
            out << med << ' ';
        }
        out << endl;
    ...
    friend istream& operator>> (istream& in, Personal& pers)
    {
        ...
        try
        {
            ...
            {
                if (!isalpha(aux[i]) && aux[i] != ' ') conditie = false;
            }
        ...
        {
            cout << "Numele trebuie sa contina doar litere si spatii.";
            aux = "anonim";
        }
        ...
        {
            in >> pers.tura;
            if (pers.tura != 'z' && pers.tura != 'n')
                throw 1;
        }
        catch (...)
        {
            cout << "Tura nu a fost introdusa corect. Se seteaza automat tura de zi.\n";
            pers.tura = 'z';
        }
        cout << "\nIntroduceti sectia ";
        in >> pers.sectie;
...
public:
    ...
    friend std::istream& operator>>(std::istream& in, Doctor& d)
    {
        ...
        {
            cout << "Valoarea booleana 'garzi' nu este 0/1. Se seteaza automat 0\n";
            d.garzi = 0;
        }
        return in;
    }
...
{
    cout << "-----------------------\n";
    cout << "1. Adaugare Doctor din fisier\n";
    cout << "2. Adaugare Asistent din fisier\n";
    cout << "3. Adaugare Pacient din fisier\n";
    cout << "4. Afisare Personal in fisier\n";
    cout << "5. Afisare Pacienti in fisier\n";
    cout << "6. Stergere Doctor\n";
    cout << "7. Stergere Asistent\n";
    cout << "8. Stergere Pacient\n";
    cout << "9. Asignare Pacient la Doctor\n";
...
int main()
{
    ...
    vector<Doctor> vectorDoctori;
    vector<Asistent> vectorAsistenti;

    map<Doctor, Pacient> spitalToplita;
    ...
    do
    {
        ...
        if (opt == 1)
        {
            ...
            while (ok_loop)
            {
                try
                {
                    ...
                    Doctor temp;
                    fin >> temp;
                    vectorDoctori.push_back(temp);

                    fin.close();
                }
                catch (...) 
                {
                    cout << "Fisierul nu a putut fi deschis. Mai incercati o data.\n\n";
                    ok_loop = true;
                }
            }
        }
        else if (opt == 2)
        {
            ...
            Asistent temp;
            fin >> temp;
            vectorAsistenti.push_back(temp);
        ...
        else if (opt == 4)
        {
            ...
            fout << "\nDoctorii:\n";
            for (int i = 0; i < vectorDoctori.size(); i++)
            {
                fout << vectorDoctori[i] << endl;
                fout << "Orele lucrate/zi: ";
                vectorDoctori[i].afisare();
            }
            fout << "\nAsistentii:\n";
            for (int i = 0; i < vectorAsistenti.size(); i++)
            {
                fout << vectorAsistenti[i] << endl;
                fout << "Orele lucrate/zi: ";
                vectorAsistenti[i].afisare();
            }
        ...
        else if (opt == 5)
        {
            ...
            {
                fout << vectorPacienti[i] << endl;
                fout << "Tratamente administrate/zi: ";
        ...
        {
            int index;
            cout << "Ce doctor doriti sa stergeti? [0 - " << vectorDoctori.size() - 1 << "]: ";
            cin >> index;
            vectorDoctori.erase(vectorDoctori.begin() + index);
        }
        else if (opt == 7)
        {
            int index;
            cout << "Ce asistent doriti sa stergeti? [0 - " << vectorAsistenti.size() - 1 << "]: ";
            cin >> index;
            vectorAsistenti.erase(vectorAsistenti.begin() + index);
        }
        else if (opt == 8)
        {
            int index;
        ...
        {
            int index1, index2;
            cout << "Alegeti un doctor [0 - " << vectorDoctori.size() - 1 << "]: ";
            cin >> index1;
            cout << "Alegeti un pacient [0 - " << vectorPacienti.size() - 1 << "]: ";
            cin >> index2;

            spitalToplita.insert(pair <Doctor, Pacient>(vectorDoctori[index1], vectorPacienti[index2]));
